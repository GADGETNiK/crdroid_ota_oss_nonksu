...
zip=$(basename $path/out/target/product/$device/crDroidAndroid-13.0-*-$device-*-GAPPS.zip)        # Here we get the package name with the extension .zip
...
gh release create $nozip --notes "Automated release crDroid (GAPPS/NON-KSU) for $device $version $date/$time" $path/out/target/product/$device/$zip
